...
// import { loadMessages } from "@/redux/slices/messageSlice";
...
// const prefetchedChatIds = useSelector(
//   (state: RootState) => (state.message as any).prefetchedChatIds || {}
// );
//   const copy = useMemo(
//     () => ({
//       searchPlaceholder:
...
//     const hasMessages =
//       Array.isArray(messagesByChatId?.[chatId]) &&
//       messagesByChatId[chatId].length > 0;

//     const alreadyPrefetched = Boolean(prefetchedChatIds?.[chatId]);

//     if (hasMessages || alreadyPrefetched) return;
...
export default function ChatListScreen() {
  ...
  const didHydrateMessagesFromCacheRef = useRef(false);
  const didSyncFirstChatsFromServerRef = useRef(false);
  ...
  useEffect(() => {
    if (didHydrateMessagesFromCacheRef.current) return;
    if (!currentUser?._id) return;
    if (!Array.isArray(chats) || chats.length === 0) return;

    didHydrateMessagesFromCacheRef.current = true;

    const hydrateMessagesFromLocalStorage = async () => {
      try {
        for (const chat of chats) {
          ...
          if (alreadyHasMessages) continue;
        ...
        console.log("hydrateMessagesFromLocalStorage error:", error);
      }
    };

    hydrateMessagesFromLocalStorage();
  }, [currentUser?._id, chats.length, messagesByChatId, dispatch]);

  useEffect(() => {
    if (didSyncFirstChatsFromServerRef.current) return;
    if (!currentUser?._id) return;
    if (!Array.isArray(chats) || chats.length === 0) return;

    didSyncFirstChatsFromServerRef.current = true;

    const syncLatestMessagesFromServer = async () => {
      try {
        const firstChats = chats.slice(0, 12);

        firstChats.forEach((chat: any) => {
          const chatId = String(chat?._id || "");
          if (!chatId) return;
        ...
        console.log("syncLatestMessagesFromServer error:", error);
      }
    };

    syncLatestMessagesFromServer();
  }, [currentUser?._id, chats.length, dispatch]);
  ...
  const onRefresh = async () => {
    ...
    try {
      ...
      didSyncFirstChatsFromServerRef.current = false;
    } catch (error) {
    } finally {
      setRefreshing(false);
    }
