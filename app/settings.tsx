...
export default function SettingsScreen() {
  ...
  const avatar = String(
    currentUser?.activeCustomization?.avatarGif ||
    currentUser?.avatarGif ||
    currentUser?.avatar ||
    ""
  );

  const cover = String(currentUser?.coverImage || currentUser?.cover || "");
const customImageBadgeUrl = String(
  currentUser?.customImageBadge?.url || ""
).trim();

const customImageBadgeActive = Boolean(
  currentUser?.customImageBadge?.isActive === true &&
    customImageBadgeUrl.length > 0
);

const hasCustomImageBadge = customImageBadgeUrl.length > 0;
  ...
  async function pickAndUploadCustomImageBadge() {
    const ok = await ensureMediaPermission();
    if (!ok) return;
    ...
    try {
      ...
      if (isGifAsset(asset)) {
        Alert.alert(
          "Image badge",
          "Please choose a static image, not GIF."
        );
        return;
      }

      await runWithLoading("Uploading image badge", async () => {
        const userId = String(currentUser?._id || currentUser?.id || "");

        const file = getUploadFileFromAsset(
          asset,
          `image-badge-${Date.now()}.jpg`
        );

        console.log("🟡 [Settings][pickAndUploadCustomImageBadge] upload start", {
          userId,
          file,
        });
        ...
        const uploaded = await uploadSingleFile({
          file,
          folder: "badges" as any,
          userId,
        });

        console.log("🟢 [Settings][pickAndUploadCustomImageBadge] uploaded", {
          uploaded,
        });
        ...
        Toast.show({
          type: "success",
          text1: t("common.success"),
          text2: "Image badge saved",
        });
      });
    } catch (error: any) {
      console.log("🔴 [Settings][pickAndUploadCustomImageBadge] error", {
      ...
      Toast.show({
        type: "error",
        text1: t("common.error"),
        text2: String(error?.message || error || "Failed to upload image badge"),
      });
    }
  }
  async function disableCustomImageBadge() {
    await runWithLoading("Removing image badge", async () => {
      try {
        await dispatch(
          updateProfile({
            customImageBadge: {
        ...
        Toast.show({
          type: "success",
          text1: t("common.success"),
          text2: "Image badge removed",
        });
      } catch (error: any) {
        Toast.show({
          type: "error",
          text1: t("common.error"),
          text2: String(error?.message || error || "Failed to remove image badge"),
        });
      }
    });
  }
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.background }]}>
      ...
      >
       ...
       {customImageBadgeActive && customImageBadgeUrl ? (
  <Image
    source={{ uri: customImageBadgeUrl }}
    style={styles.customImageBadge}
    contentFit="contain"
  />
) : null}
          </View>
