...
import LottieBadge from "@/components/LottieBadge";
import BadgeLottiePickerModal from "@/components/store/BadgeLottiePickerModal";
import StoreActionRow from "@/components/storeScreen/StoreActionRow";
import StoreBalanceCard from "@/components/storeScreen/StoreBalanceCard";
import StoreCategoriesGrid from "@/components/storeScreen/StoreCategoriesGrid";
import StoreSectionTitle from "@/components/storeScreen/StoreSectionTitle";
import StoreStatsGrid from "@/components/storeScreen/StoreStatsGrid";
import { buildCategoryCards, CREATE_ACCOUNT_COST, CUSTOM_EMOJI_BADGE_COST } from "@/components/storeScreen/constants";
import {
  formatCoinz,
  formatDate,
  formatMoney,
  ...
  isExpired,
  prettyType,
} from "@/components/storeScreen/helpers";
import type { CoinzPack, ModalState, StoreActionItem, StoreType } from "@/components/storeScreen/types";
import { AppTheme, Colors, Fonts } from "@/constants/theme";
import { useColorScheme } from "@/hooks/use-color-scheme";
import { useTranslation } from "@/hooks/useTranslation";
import { useAppDispatch, useAppSelector } from "@/redux/hooks";
...
const [badgePickerType, setBadgePickerType] = useState<"lottie" | "image">("lottie");
  ...
  const categoryCards = useMemo(() => buildCategoryCards(t), [t]);
...
const imageBadgeItems = useMemo(() => {
  ...
  const stats = useMemo(() => getStoreStats(my), [my]);

  const activeAccountColor = useMemo(() => {
    const value = String(active?.usernameColor || "").trim();
    return value ? value : "0 Days";
  }, [active?.usernameColor]);
  ...
  const storeActions: StoreActionItem[] = useMemo(
    () => [
      {
        key: "buy",
        title: "Buy TCoins",
      ...
      {
        key: "accountColor",
        title: `Account Color - ${activeAccountColor}`,
        icon: "text-outline",
        tone: "cyan",
        onPress: () => openCategoryModal("usernameColor"),
      },
      {
        key: "customEmoji",
        title: t("storeScreen.customEmoji.cardTitle"),
        subtitle: customEmojiBadge?.emoji
          ? `${t("storeScreen.customEmoji.current")} ${customEmojiBadge.emoji}`
          : t("storeScreen.customEmoji.cardDesc"),
        icon: "happy-outline",
        tone: "violet",
        value: customEmojiBadge?.emoji || "🙂",
        onPress: () => {
          setCustomEmojiInput(customEmojiBadge?.emoji || "");
          setCustomEmojiSetActive(true);
          setModal((s) => ({ ...s, customEmoji: true }));
      ...
      },
      {
        ...
        icon: "time-outline",
        tone: "neutral",
        onPress: () => setModal((s) => ({ ...s, transactions: true })),
      },
      {
        key: "earnings",
        title: "Earnings",
        icon: "logo-usd",
        tone: "gold",
        value: `${formatMoney(stats.earningsUsd)}$`,
        onPress: () => setModal((s) => ({ ...s, earnings: true })),
      },
      {
        key: "inventory",
        title: t("storeScreen.inventory.title"),
        subtitle: `${my?.inventory?.length || 0} Items`,
    ...
    [
      activeAccountColor,
      customEmojiBadge?.emoji,
      imageBadgeItems.length,
      lottieBadgeItems.length,
      my?.inventory?.length,
      openCategoryModal,
      stats.earningsUsd,
      t,
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      ...
      <ScrollView
        ...
        contentContainerStyle={styles.storeHomeContent}
      >
        <StoreBalanceCard
          theme={theme}
        ...
        <StoreSectionTitle theme={theme} title="This month" />
        <StoreStatsGrid theme={theme} stats={stats} />

        <StoreSectionTitle theme={theme} title="General" />
        ...
        <StoreSectionTitle
          theme={theme}
          title={t("storeScreen.sections.store")}
          subtitle="اختر القسم الذي تريد شراءه أو تفعيله"
        />

        {itemsLoading || myLoading || loadingCustomEmojiBadge ? (
          <View style={styles.homeInlineLoader}>
            <ActivityIndicator />
          </View>
        ) : null}
        ...
        <StoreCategoriesGrid
          ...
          onPressCategory={openCategoryModal}
        />
      </ScrollView>
...
/>
      ...
      <ModalShell
        visible={modal.transactions}
        onClose={() => setModal((s) => ({ ...s, transactions: false }))}
        title="Transactions history"
        theme={theme}
      >
        <View style={styles.modalBody}>
          <View style={[styles.innerCard, { backgroundColor: theme.surface2, borderColor: theme.border }]}>
            <Text style={[styles.innerTitle, { color: theme.text }]}>قريبًا</Text>
            <Text style={[styles.innerSub, { color: theme.mutedText }]}>
              اربط هنا API سجل المعاملات عند إضافته في الباك، وسيظهر داخل هذا المودال.
            </Text>
          </View>
        </View>
      </ModalShell>
      ...
      <ModalShell
        visible={modal.earnings}
        onClose={() => setModal((s) => ({ ...s, earnings: false }))}
        title="Earnings"
        theme={theme}
      >
        <View style={styles.modalBody}>
          <View style={[styles.innerCard, { backgroundColor: theme.surface2, borderColor: theme.border }]}>
            <Text style={[styles.innerTitle, { color: theme.text }]}>
              {formatMoney(stats.earningsUsd)}$
            </Text>
            <Text style={[styles.innerSub, { color: theme.mutedText }]}>
              قيمة الأرباح الحالية حسب البيانات المتاحة في حساب المستخدم.
            </Text>
          </View>
        </View>
      </ModalShell>
