...
export default function TweetDetailsScreen() {
  ...
  const { currentTweet, comments, loading, likesUsers, likesUsersLoading } = useSelector(
    (state: RootState) => state.tweets
  );
  const { user } = useSelector((state: RootState) => state.auth);
  ...
  const [likesModalTitle, setLikesModalTitle] = useState('');
  const keyboardHeight = useSharedValue(0);
  ...
  const openTweetLikes = async () => {
    if (!currentTweet?._id) return;
    setLikesModalTitle(
      language === 'ar' ? 'الأشخاص الذين أعجبوا بهذا المنشور' : 'People who liked this post'
    );
    setShowLikesModal(true);
    await dispatch(getTweetLikesUsers(currentTweet._id));
  };

  const openCommentLikes = async (commentId: string) => {
    setLikesModalTitle(
      language === 'ar' ? 'الأشخاص الذين أعجبوا بهذا التعليق' : 'People who liked this comment'
    );
    setShowLikesModal(true);
    await dispatch(getCommentLikesUsers(commentId));
  };
  ...
  const handleLoadReplies = async (commentId: string) => {
    try {
      ...
      await dispatch(getCommentReplies(commentId));
    } finally {
  ...
  const renderCommentItem = (item: CommentItem, depth = 0): React.ReactNode => {
    const hasReplies = (item.repliesCount || 0) > 0;
    const loadedReplies = item.replies || [];
        ...
        <View
          style={[
            styles.replyRow,
            {
              borderBottomColor: theme.separator,
              flexDirection: isRTL ? 'row-reverse' : 'row',
              paddingLeft: isRTL ? 12 : 12 + depth * 22,
              paddingRight: isRTL ? 12 + depth * 22 : 12,
            },
          ]}
        >
          <View style={styles.replyAvatarWrap}>
          ...
          <View style={styles.replyBody}>
            ...
            >
              <TouchableOpacity
                activeOpacity={0.85}
                style={styles.replyMiniBtn}
                onPress={() => handleStartReply(item)}
              >
                <Ionicons name="chatbubble-outline" size={16} color={theme.mutedText} />
                <Text style={[styles.replyMiniText, { color: theme.mutedText }]}>
                  {formatCount(item.repliesCount)}
                </Text>
              </TouchableOpacity>

              <TouchableOpacity activeOpacity={0.85} style={styles.replyMiniBtn}>
                <Ionicons name="repeat-outline" size={16} color={theme.mutedText} />
  ...
  >
    ...
    >
      {formatCount(item.likesCount)}
            ...
            {!isReplyLevel && hasReplies && !isRepliesLoaded && (
              <TouchableOpacity
                activeOpacity={0.85}
                style={styles.loadRepliesBtn}
                onPress={() => handleLoadReplies(item._id)}
              >
                {isLoadingReplies ? (
                  ...
                  <Text
                    style={[
                      ...
                      { color: theme.primary, textAlign: isRTL ? 'right' : 'left' },
                    ]}
                  >
                    {language === 'ar'
                      ? `عرض الردود (${formatCount(item.repliesCount)})`
                      : `View replies (${formatCount(item.repliesCount)})`}
                  </Text>
                )}
              </TouchableOpacity>
            )}
            ...
            {!!loadedReplies.length && (
              <View style={styles.repliesWrap}>
                {loadedReplies.map((reply) => renderCommentItem(reply, depth + 1))}
              </View>
            )}
          </View>
        </View>
  ...
  const renderComment = ({ item }: { item: CommentItem }) => {
    return <>{renderCommentItem(item, 0)}</>;
  };
              ...
              >
                ...
                <View style={styles.postMain}>
                  ...
                  >
                    ...
                    <TouchableOpacity onPress={openTweetLikes} activeOpacity={0.85}>
                      <Text style={[styles.statsText, { color: theme.text }]}>
                        {formatCount(currentTweet.likesCount)} {t('tweetDetailsScreen.likes')}
                      </Text>
                    </TouchableOpacity>
          ...
          {!!replyingTo && (
            ...
            >
              <View style={{ flex: 1 }}>
                ...
                >
                  {language === 'ar'
                    ? `الرد على ${replyingTo.user?.username || replyingTo.user?.atUsername || 'user'}`
                    : `Replying to ${replyingTo.user?.username || replyingTo.user?.atUsername || 'user'}`}
                </Text>
              </View>
      ...
      <Modal visible={showMenu} transparent animationType="fade">
        ...
        <Pressable style={styles.modalOverlay} onPress={closeLikesModal}>
          ...
          >
            <View
              style={[
                styles.likesModalHeader,
                { borderBottomColor: theme.separator, flexDirection: isRTL ? 'row-reverse' : 'row' },
              ]}
            >
              <Text
                style={[
                  styles.likesModalTitle,
                  { color: theme.text, textAlign: isRTL ? 'right' : 'left' },
                ]}
              >
                {likesModalTitle}
              ...
              <TouchableOpacity onPress={closeLikesModal} activeOpacity={0.85}>
            ...
            {likesUsersLoading ? (
              ...
              <View style={styles.likesEmptyWrap}>
                <Text style={[styles.likesEmptyText, { color: theme.mutedText }]}>
                  {language === 'ar' ? 'لا يوجد إعجابات بعد' : 'No likes yet'}
                </Text>
              </View>
            ) : (
              <FlatList
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    {item.avatar ? (
                      <Image source={{ uri: item.avatar }} style={styles.likeUserAvatar} />
                    ) : (
                      <View
                        style={[
                          styles.likeUserAvatarPlaceholder,
                          {
                            backgroundColor: theme.primarySoft,
                            borderColor: theme.border,
                          },
