...
export default function RoomSettingsScreen() {
  ...
  // ✅ Theme (Light/Dark)
  // إذا لديك hook خاص useColorScheme عندك في المشروع استخدمه بدل التالي.
  // هنا نستخدم react-native مباشرة لتفادي أي تعارض.
  const rnScheme = require("react-native").useColorScheme?.() as ThemeName | null;
  const theme: AppTheme = Colors[(rnScheme === "dark" ? "dark" : "light") as ThemeName];
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView style={styles.container} contentContainerStyle={styles.containerContent}>
        ...
        {/* Boost */}
        <Section theme={theme} styles={styles} title="Boost" icon="flash-outline">
          <Field
            theme={theme}
            styles={styles}
            label="Boost Level (0-10)"
          ...
          >
            {saving ? (
              ...
              <Text style={styles.primaryBtnText}>تنفيذ Boost</Text>
            )}
          </TouchableOpacity>
        </Section>
