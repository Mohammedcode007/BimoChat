...
/* ================= GIFTS ================= */
type GiftItem = { key: string; title: string; icon: string; price?: number };
...
export default function ChatScreen() {
  ...
  const pickSenderFromMessage = (m: any) => {
    ...
    return { senderId, username, avatar, snapshotRole: snapshotRole || undefined, activeBadges, verificationType };
  };
  ...
  const mapReduxToUIMessage = (m: any): MessageUI => {
    ...
    const buildReplyPreview = (raw: any): MessageUI | undefined => {
      ...
      if (typeof raw === "object") {
        ...
        const rType = String(raw?.type || "text");
        const uiT: MessageUI["type"] =
          rType === "image" ? "image" : rType === "video" ? "video" : rType === "audio" ? "audio" : rType === "file" ? "file" : "text";
      ...
      if (typeof raw === "string") {
        ...
        const refType = String(ref?.type || "text");
        const uiT: MessageUI["type"] =
          refType === "image" ? "image" : refType === "video" ? "video" : refType === "audio" ? "audio" : refType === "file" ? "file" : "text";
    ...
    <SafeAreaView style={styles.root}>
      ...
      <Modal transparent visible={showActions} animationType="fade" onRequestClose={() => setShowActions(false)}>
        <View style={styles.actionsOverlay}>
          <View style={styles.actionsBox}>
            <View style={styles.reactionsRow}>
              {REACTIONS.map((r) => (
                <TouchableOpacity key={r} onPress={() => selectedMessage && addReaction(selectedMessage.id, r)} activeOpacity={0.85}>
                  <Text style={{ fontSize: 22 }}>{r}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            {(selectedMessage?.sender?.id === myUserId || canModerate) &&
              selectedMessage?.type !== "system" &&
              !selectedMessage?.deletedForEveryone && (
                <TouchableOpacity onPress={() => selectedMessage && deleteMessage(selectedMessage.id)} activeOpacity={0.85}>
      ...
      {String(giftOverlay.giftKey || "").startsWith("boost") ? (
        <RocketBoostOverlay
          ...
          onDone={() => {
            if (giftOverlay.messageId) markGiftDone(giftOverlay.messageId);
            setGiftOverlay({ visible: false, messageId: null, giftKey: null, icon: "🎁", count: 45 });
          }}
        />
      ) : (
        <GiftBurstOverlay
