...
export default function RegisterScreen() {
  ...
  const validate = (u: string, p: string, c: string): FieldErrors => {
    const next: FieldErrors = {};
    const uTrim = u.trim();
  ...
  const handleRegister = async () => {
    ...
    const v = validate(username, password, confirm);

    if (v.username || v.password || v.confirm) {
      setErrors(v);
      triggerErrorAnim();
    ...
    try {
      ...
      const isBlocked = String(payload?.code || "").toUpperCase() === "BLOCKED";
      ...
      const isBlocked = String(data?.code || "").toUpperCase() === "BLOCKED";
