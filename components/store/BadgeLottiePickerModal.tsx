...
export default function BadgeLottiePickerModal({
  ...
  useEffect(() => {
    ...
    if (selectedKey) {
      setPickedKey(String(selectedKey));
      return;
    }

    const firstKey = String(items?.[0]?.key || "");
    setPickedKey(firstKey);
  }, [visible, selectedKey, items]);
  ...
  const renderBadgePreview = (item: any, size: number) => {
    const lottieUrl = String(item?.meta?.lottieUrl || "").trim();
    const imageUrl = String(getItemImageUrl(item) || "").trim();

    if (lottieUrl) {
      return <LottieBadge url={lottieUrl} size={size} />;
    }
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {pickerTitle || t("storeScreen.badgePicker.title")}
            </Text>
          ...
          >
            {pickerTitle === "Lottie Badges"
              ? "Animated JSON badges"
              : pickerTitle === "Image Badges"
                ? "Static image badges"
                : t("storeScreen.badgePicker.subtitle")}
          </Text>
          ...
          >
            {!selectedItem ? (
              ...
              <View style={{ flexDirection: row, gap: 12, alignItems: "center" }}>
                ...
                >
                  {renderBadgePreview(selectedItem, 58)}
                </View>
